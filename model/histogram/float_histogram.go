...
// not exist in the receiving histogram are inserted into the latter. This
// method returns a pointer to the receiving histogram for convenience.
//
...
// compared to the provided previous histogram. It also returns true if any
// count (in any bucket, in the zero count, or in the count of observations, but
// NOT the sum of observations) is smaller in the receiving histogram compared
// to the previous histogram. Otherwise, it returns false.
//
// IMPORTANT: This method requires the Schema and the ZeroThreshold to be the
// same in both histograms. Otherwise, its behavior is undefined.
// TODO(beorn7): Change that!
...
func detectReset(currIt, prevIt FloatBucketIterator) bool {
	...
	if !currIt.Next() {
		return true // No bucket in current, but at least one in previous histogram.
	}
	...
	for {
		...
		if currBucket.Index > prevBucket.Index {
			return true // Previous histogram has buckets current one does not have.
		}
